...
def main():
    ...
    CSV_PATH = r"E:\PAIR360\Traversal2\1st_Dormitory\Sequence0\CAM.csv"
    IMG_DIR = r"E:\PAIR360\Traversal2\1st_Dormitory\Sequence0\T2-1st_Dormitory-0-stitching\stitching"
    PCD_DIR = r"E:\PAIR360\Traversal2\1st_Dormitory\Sequence0\T2-1st_Dormitory-0-pcd\pcd"
    EPOCHS = 30
    BATCH_SIZE = 32
    ...
    if glob.glob(CSV_PATH):
        ...
        print(f"Entraînement lancé sur {device} pour {EPOCHS} époques...")
        for epoch in range(EPOCHS):
            model.train()
            epoch_loss = 0
            for img, target_pcd in tqdm(dataloader, desc=f"Epoch {epoch+1}/{EPOCHS}"):
                ...
                pred = output_z[:, :256].reshape(-1, 256)
                target = target_pcd[:, :, 2][:, :256].reshape(-1, 256)
                ...
                #
                # for b in range(B):
                #     pred = output_z[b].flatten()[:256]
                #     target = target_pcd[b, :, 2][:256]
                #
                #     loss += criterion(pred, target)
                #
                # loss = loss / B
